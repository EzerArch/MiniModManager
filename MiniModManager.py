...
# Variables
fileext_cfg = '.cfg'  # config file extension, it can be changed later
...
def cfgMan(path):
    ...
    if lst_cfgs:
        ...
        for path_cfg in lst_cfgs:
            print('[', n, ']', os.path.split(path_cfg)[1][:-4])  # name file without extension
            n += 1
...
str_suffix: file extension suffix
"""
    # Prompt
    print("\n-- Renaming files --")
...
if option == '1':
    ...
    files = listFiles(path_mod, file_exts_x)  # files to rename back
    if files:
        print('Your mod folder contains renamed files! Renaming them back... ')
        renFiles_back(path_mod, files, file_exts_x)
